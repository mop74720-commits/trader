...
当前首页为**美股情报工作台**：参考 [牛牛1号](https://niuone.cn/) 的浅色、横向导航、多面板布局，默认关注美股公司与美国宏观政策。新闻与模拟账户的资产范围独立，模拟账户仍为原有 BTC / ETH / SOL 合成行情。

- 默认自动读取 CNBC、MarketWatch、白宫与美联储 RSS；加密资产来源保留配置，默认关闭。
- 支持特朗普 / 马斯克 / 鲍威尔、关税与政策、财报分类，以及美股范围、新闻 / 原帖、历史时点筛选。
- 特朗普 X、马斯克 X 已提供逐账号查询配置，**默认未启用**。X API 可能计费，需用户自行授权、配置凭据后启用；本次没有调用付费接口。
- 特朗普 Truth Social 提供账号直达与本地导入，**没有实时自动采集**。公开接口在本环境返回 403。导入方式见 [信息来源说明](docs/intelligence.md#关键人物与美股阅读范围)。
- 人物被提及、账户原帖、导入社交帖分开标注；原帖也不等于事实已核实。暂无真实美股行情，不展示虚构的行情涨跌或消息价格影响。
...
- 默认规则引擎，**没有模型请求、真实行情请求或交易所订单**。默认启用独立公共情报采集（CNBC、MarketWatch、白宫与 Fed RSS）；加 `--no-intelligence` 可完全关闭情报联网。
